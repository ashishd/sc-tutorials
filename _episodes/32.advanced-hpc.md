...
```bash
singularity pull docker://quay.io/pawsey/sc22-openfoam:v2012
```
...
```bash
singularity exec sc22-openfoam_v2012.sif bash -c 'ldd $(which simpleFoam) |grep libmpi'
```
```output
...
```
```output
	libmpi.so.12 => /opt/intel/oneapi/mpi/2021.1.1/lib/release/libmpi.so.12 (0x00007f43d78b0000)
```
...
NOTE: this part was executed on the Zeus cluster at Pawsey.  You can follow the outputs here.
